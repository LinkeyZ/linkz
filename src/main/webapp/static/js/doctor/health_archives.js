$(document).ready(function(){
	//下拉列表
	$(".select_input").unbind("click");
	$(".select_input").click(function(event) {
		var this_class=$(this).attr("class");
		if(this_class.indexOf("can_drop")!=-1){
	...
	//点击下拉列表item
	$(".select .select_choose li").unbind("click");
	$(".select .select_choose li").click(function(event) {
		var show_value=$(this).html();
		$(this).closest('.select_choose').prev().val(show_value);
		var value = $(this).attr("data");
		$(this).closest('.select_choose').prev().attr('data',value);
		$(this).closest('.select_choose').css("display","none");
		$(this).closest('.select_choose').prev().removeClass('input_div_text_blue');
		$(this).closest('.select_choose').prev().addClass('input_div_text_gray');
	});
	$("#jibing").unbind("click");
	...
	$("#btn_jibing").click(function(event) {
		...
		$("#add_jibing_kuang .bottom table td ul li").each(function(index, el) {
			...
			if(class_check=="cur"){
				...
				str_jbbm += $(this).attr("data")+",";
			}			
		});
		str_jibing=str_jibing.substring(0,str_jibing.length-1);
		str_jbbm = str_jbbm.substring(0,str_jbbm.length-1);
		$("#jibingbm").val(str_jbbm);
		$("#show_jibing span").text(str_jibing);
	});
	
	//客户类别
	$("#clas_customer").unbind("click");
	$("#class_customer").click(function(event) {
		var class_check=$(this).attr("class");
		if(class_check.indexOf("can_drop") != -1){
			$(".heian").css("display","block");
			$("#add_customer_kuang").css("display","block");
		}
	});
	//增加病史
	$("#btn_bingshi").unbind("click");
	$("#btn_bingshi").click(function(event) {
		$(".close").trigger('click');

		var str_all_bingzheng="";//循环出所有病症
		var str_all_bingzhengbm = "";
		$("#add_bingshi_kuang .bottom table tr").each(function(index, el) {
			var index_tr=$(this).index();
			//不是第一行
			if(index_tr!=0){
				str_all_bingzheng+=$(this).children('td').html()+"|";
				str_all_bingzhengbm+=$(this).children('td').attr("data")+"|";
			}
		});
		...
		var arry_bingzhengbm = str_all_bingzhengbm.split("|");
		var str_Info="";
		...
		for (var i = 1; i <= arry_bingzheng.length; i++) {
			...
			$('#add_bingshi_kuang .bottom table tr:eq('+i+') td').each(function(index, el) {
				...
				if(index_td!=0){//判断td是否有span(即不是病症)
					...
					if(class_check=="cur"){
						...
						bingshibm = bingshibm + jbbm+","+arrbm[index_td-1]+";";
					}
				}
			...
			bingshibmstr += bingshibm;
		};
		$("#bingshibm").val(bingshibmstr);
		str_info=str_Info.substring(0,str_Info.length-1);
	});
	/*$(".edit_beizhu_jibing").unbind("click");
	$(".edit_beizhu_jibing").click(function(event) {
		var class_check=$(this).attr("class");
		if(class_check.indexOf("can_drop") != -1){
			var edit=$(this).html();
			if(edit=="编辑"){
	...
	$(".txt_customer").attr("disabled",true);
//	$(".edit_beizhu").attr("disabled",true);
	$(".txt_family").attr("disabled",true);
	
	$(".modify_archive").click(function(){
		$(this).css("display","none");
		$(".return_archive").css("display","block");
		$(".select_input").addClass("can_drop");
		$(".txt_customer").attr("disabled",false);
		$(".txt_family").attr("disabled",false);
		$("#jibing").addClass("can_drop");
		$("#bingshi").addClass("can_drop");
		$("#class_customer").addClass("can_drop");
		personInfoSave();
	});
	$(".return_archive").click(function(){
		$(this).css("display","none");
		$(".modify_archive").css("display","block");
		$(".select_input").removeClass("can_drop");
		$(".txt_customer").attr("disabled",true);
		$(".txt_family").attr("disabled",true);
		$("#jibing").removeClass("can_drop");
		$("#bingshi").removeClass("can_drop");
		$("#class_customer").removeClass("can_drop");
		$("#personalinfoedit").unbind("click");
	});
	//增加客户类别
	$("#btn_customer").click(function(event) {
		...
		var str_customer="";
		//已选择有角色
		if($("#add_customer_kuang .select_r option").size()>0){
			$("#add_customer_kuang .select_r option").each(function(index, el) {
				str_customer+=$(this).html()+"，";
			});
			$("#show_customer span").text(str_customer.substring(0,str_customer.length-1));
		}
	});
	function initData(){
		$("#customer_tab li").each(function(index,el){
			$(this).removeClass("cur");
		});
		$("#customer_tab ul li").each(function(index,el){
			$(this).removeClass("current");
		});
		$("#customer_tab li:nth-child("+2+")").addClass("cur");
		$("#customer_tab ul li:nth-child("+1+")").addClass("current");
		$(".return_archive").css("display","none");
		var monthval = $("#month").val() || "";
		var dayval = $("#day").val()||"";
		if(monthval != ""){
			$("#month").val(parseInt(monthval));
			$("#month").attr("data",parseInt(monthval));
		}
		if(dayval != ""){
			$("#day").val(parseInt(dayval));
			$("#day").attr("data",parseInt(dayval));
		}
		var dieases = $("#jibingbm").val();
		$("#add_jibing_kuang .bottom table td ul li").each(function(index, el) {
			var data = $(this).attr("data");
			if(dieases.indexOf(data) != -1){
				$(this).addClass("cur");
			}
		});
		$("#btn_jibing").trigger("click");
		var str_all_bingzheng="";//循环出所有病症
		var str_all_bingzhengbm = "";
		var familyHistory = $("#bingshibm").val();
		var familyHistories = familyHistory.split(";");
		for(k=0; k<familyHistories.length-1; k++){
			var tmp = familyHistories[k];
			var tmps = tmp.split(",");
			$("#add_bingshi_kuang .bottom table tr").each(function(index, el) {
				var index_tr=$(this).index();
				//不是第一行
				if(index_tr!=0){
					//str_all_bingzheng+=$(this).children('td').html()+"|";
					bm=$(this).children('td').attr("data");
					if(bm==tmps[0]){
						$(this).children().eq(parseInt(tmps[1])).find("span").addClass("cur");
					}
				}
			});
		}
		$("#btn_bingshi").trigger("click");
	...
	function personInfoSave(){
		$("#personalinfoedit").click(function(){
			...
			flag = showMsg(id,"系统错误!");
			var name = $('#custom_name').val() || '';
			flag = showMsg(name,'姓名');
			if(flag){
				return;
			}
			var phoneNumber = $("#custom_phoneNumber").val() || '';
			flag = showMsg(phoneNumber,'电话号码');
			if(flag){
				return;
			}
			if(flag){
			...
			if($("#add_customer_kuang .select_r option").size()>0){
				$("#add_customer_kuang .select_r option").each(function(index, el) {
					multiSelect.push($(this).attr("value"));
				});
			};
			var rolesid = multiSelect.join(",");
			flag = showMsg(rolesid,'客户类别');
			if(flag){
				return;
			}
			var serviceType = $("#serviceType").attr("data");
			var sex = $("#sexType").attr("data");
			var year = $("#year").attr("data");
			var month = formatDate($("#month").attr("data"));
			var day = formatDate($("#day").attr("data"));
			var nation = $("#nationname").attr("data");
			var marriage = $("#marriage").attr("data");
			var medicalType = $("#medicaltype").attr("data");
			var eduationType = $("#educationType").attr("data");
			var workType = $("#workType").attr("data");
			var sfzh = $("#sfzh").val();
			var province = $("#provincename").val();
			var city = $("#cityname").val();
			var quyu = $("#quyuname").val();
			var address = $("#address").val();;
			var diseases = $("#jibingbm").val();
			var bingshi = $("#bingshibm").val();
			var comments = $("#comments").val();
			var medicalExmnum = $("#medicalExmnum").val();
			//address = province+","+city+","+quyu+",@"+$("#address").val();
			$.ajax({
				url:'/doctor/customer/health/save_archives',
				data:{
					personalid:id,
					name:name,
					sex:sex,
					...
					nation:nation,
					medicare:medicalType,
					education:eduationType,
					job:workType,
					workUnit:company,
					IDCard:sfzh,
					province:province,
					city:city,
					area:quyu,
					medicalExmnum:medicalExmnum,
					comments:comments,
					homeAddress:address,
					birthday:year+"-"+month+"-"+day,
					diseases:diseases,
					familyHistory:bingshi,
					rolesid:rolesid
				},
				type:'post',
				dataType:'json',
	...
	function showMsg(value,msg,flag){
		if(value === ''){
			if(flag){
				alert(msg);
				return true;
	...
	//客户类别选择
	$("body").on("click",".select_r option",function(event) {
		var value = $(this).attr("value");
		var selectValue=$(this).html();
		if (typeof(selectValue) == 'string') {
			$(".select_l").append('<option value=\"'+value+'\">' + selectValue + '</option>');
		}
		$(this).remove();
	});
	$("body").on("click",".select_l option",function(event) {
		var value = $(this).attr("value");
		var selectValue=$(this).html();
		if (typeof(selectValue) == 'string') {
			$(".select_r").append('<option value=\"'+value+'\">' + selectValue + '</option>');
		}
		$(this).remove();
	});
...
{name:'澳门', areaList:['澳门']}
	...
	var first = provinceList;
	var second = provinceList[0].cityList;
	var third = provinceList[0].cityList[0].areaList;
	var widths = new Array();
	Array.max = function(array){
		return Math.max.apply(Math,array);
	}
	//初始化
	var p = $("#provincename").val()||"";
	if(p==""){
		$("#provincename").val(first[0].name);
	}else{
		for(y=0;y<first.length;y++){
			var item = first[y];
			if(p == item.name){
				second = provinceList[y].cityList;
				$("#provinceid").val(y);
				break;
			}
		}
	...
	var c = $("#cityname").val()||"";
	if(c==""){
		$("#cityname").val(second[0].name);
	}else{
		for(z=0;z<second.length;z++){
			var item = second[z];
			if(c == item.name){
				third = second[z].areaList;
				break;
			}
		}
	}
	...
	var q = $("#quyuname").val()||"";
	if(q==""){$("#quyuname").val(third[0]);}
	
	for(i=0; i<first.length;i++){
		$("#provinceul").append("<li>"+first[i].name+"</li>");
	}//需要先初始化 在click方法中初始化会找不到
	for(j=0; j<second.length;j++){
		var cityname = second[j].name;
		$("#cityul").append("<li>"+cityname+"</li>");
		widths[j] = cityname.length*15+20;
	}
	for(k=0; k<third.length;k++){
		$("#quyuul").append("<li>"+third[k]+"</li>");
	}
	//省级点击
	$("#province ul li").click(function(){
		
		var provincename = $(this).text();
		var provincenum = $(this).index();
		$("#provinceid").val(provincenum);
		var second = provinceList[provincenum].cityList;
		var third = provinceList[provincenum].cityList[0].areaList;
		$("#provincename").val(provincename);
		$("#cityname").val(second[0].name);
		$("#quyuname").val(third[0]);
		...
		for(k=0;k<second.length;k++){
			var cityname = second[k].name;
			$("#cityul").append("<li>"+cityname+"</li>");
			max_width = (max_width < cityname.length? cityname.length:max_width);
		}
		if(max_width > 5){
			$("#cityul li").width("160px");
		}
		/*var max_width = Array.max(widths);
		if(max_width < 90){
		...
		$("#quyuul").text('');
		for(m=0;m<third.length;m++){
			var quyuname = third[m];
			$("#quyuul").append("<li>"+quyuname+"</li>");
		}
		$("#city ul li").click(function(){
			var cityname = $(this).text();
			var provinceid = $("#provinceid").val()||0;
			$("#cityname").val(cityname);
			var citynum = $(this).index();
			$("#cityid").val(citynum);
			var third = provinceList[provinceid].cityList[citynum].areaList;
			$("#quyuul").text("");
			$("#quyuname").val(third[0]);
			var max = 0;
			for(j=0;j<third.length;j++){
				var quyuname = third[j];
				$("#quyuul").append("<li>"+third[j]+"</li>");
				max = (max < quyuname.length ? quyuname.length:max);
			}
			if(max > 5){
				$("#quyuul li").width("160px")
			}
			
			$("#quyu ul li").click(function(){
				$(this).closest('.select_choose').css("display","none");
				var quyuname = $(this).text();
				$("#quyuname").val(quyuname);
			});
			$(this).closest('.select_choose').css("display","none");
		});
		$("#quyu ul li").click(function(){
	...
	/*$("#city ul li").click(function(){
		$("#city intput").attr("value",second[$(this).index()].name);
		third = second[$(this).index()].areaList;
		$("#quyuul").text("");
		$("#quyu intput").attr("value",third[0]);
		for(i=0;i<third.length;i++){
			$("#quyuul").append("<li>"+third[i]+"</li>");
			$("#quyu ul li").click(function(){
				$("#quyu intput").attr("value",third[$(this).index()]);
			});
		}
	});*/
	//点击其他地方小时弹层
	...
	$('html,body').click(function(e){
		if($(e.target).is($(".select_input"))||$(e.target).html()==""){
			return;
		}else{
