...
<div class="row">
	<div class="col-xs-12">
		<!-- PAGE CONTENT BEGINS -->
		<form id="validation-form" class="form-horizontal" role="form" action="/admin/equipmentInfo/edit" method="post">
		  <div class="form-group">
					...
					<div class="col-sm-4">
						...
						<input name="orgaId" id="organization" value="${organ.id }" type="hidden"/>
						</div>
					</div>
			...
			<div class="form-group">
				...
				<input name="equipmentId"  value="${equipmentInfo.equipmentId }" type="hidden"/>
				<input name="id"  value="${equipmentInfo.id }" type="hidden"/>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
				<input name="equipmentName" value="${equipmentInfo.equipmentName }" type="text" id="form-field-1" placeholder="请输入设备名称" class="col-xs-10 col-sm-5" />
				</div>
			</div>
				...
				<div class="col-sm-9">
					<select name="equipmentType" class="col-xs-10 col-sm-5" id="form-field-select-1" data-placeholder="选择设备类型" >
						<c:forEach var="type" items="${typeList}">
							<option value="${type.typeId}" <c:if test="${type.typeId eq equipmentInfo.equipmentType}">selected="selected"</c:if> >${type.typeName}</option>
						</c:forEach>
					</select>
				</div>
			</div>
				...
				<div class="col-sm-9">
					<select  name="manufacturer"  class="col-xs-10 col-sm-5" id="form-field-select-1" data-placeholder="选择生产厂商">
						<c:forEach var="mf" items="${mfList}">
							<option value="${mf.id}" <c:if test="${mf.id eq equipmentInfo.manufacturer}">selected="selected"</c:if>>${mf.name}</option>
						</c:forEach>
					</select>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-9"> 
				  <input type="radio" value=1  <c:if test="${1 eq equipmentInfo.orPublic}">checked="checked"</c:if> class="ace" name="orPublic" onclick="hideLimitDiv()"><span class="lbl">公用</span>&nbsp;&nbsp;
				  <input type="radio" value=2  <c:if test="${2 eq equipmentInfo.orPublic}">checked="checked"</c:if> class="ace" name="orPublic" onclick="showLimitDiv()"><span class="lbl">非公用</span>
				</div>
			</div>
			<div class="form-group" id="limitDiv" style="display: none;">
				...
				<div class="col-sm-9">
				<input name="equipmentBindLimit" type="text" value="${equipmentInfo.equipmentBindLimit }"  id="form-field-1" placeholder="请输入设备用户绑定上限人数" class="col-xs-10 col-sm-5" />
				</div>
			</div>
...
$(window).load(function(){
	...
	var orPublict='${equipmentInfo.orPublic}';
	if(orPublict==2){
		showLimitDiv();
	}
});
