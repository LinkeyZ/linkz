...
<div class="row">
	<div class="col-xs-12">
		...
		<form id="validation-form" class="form-horizontal" role="form"
			action="/goods/updateGoods" method="post">
				...
				<div class="col-sm-9">
					<div class="clearfix">
						<input name="goodsName" type="text" id="goodsName"
							placeholder="" class="col-xs-10 col-sm-5"  value="${goods.goodsName}"/>
					</div>
				</div>
			</div>
					...
					<div class="clearfix">
						<select name="goodsTypeId" class="col-xs-10 col-sm-5"
							...
							<c:forEach var="item" items="${ typeList }">
								<c:choose>
									<c:when test="${item.goodsTypeName == goods.goodsType.goodsTypeName }">
										<option selected="selected" value = "${item.goodsTypeId}">${item.goodsTypeName}
									</c:when>
									<c:otherwise>
